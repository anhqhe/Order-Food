...
export default function DeliveryOrdersScreen() {
  ...
  const handleUpdateStatus = async (orderId: string, status: "delivering" | "completed") => {
    try {
      await deliveryAPI.updateOrderStatus(orderId, status);
      Alert.alert(
        "Thành công",
        status === "delivering" ? "Đã chuyển sang chờ giao hàng" : "Xác nhận đơn hàng thành công"
      );
      loadData();
    } catch (error: any) {
  ...
  const renderOrderItem = ({ item }: { item: Order }) => {
    const config = STATUS_CONFIG[item.status] || { label: item.status, color: "#888" };
    const canSetDelivering = item.status === "confirmed";
    const canSetCompleted = item.status === "delivering";
        ...
        <View style={styles.actionRow}>
          {canSetDelivering && (
            <TouchableOpacity
              ...
              onPress={() => handleUpdateStatus(item._id, "delivering")}
            >
              <Ionicons name="bicycle-outline" size={18} color="#FFF" />
              <Text style={styles.actionBtnText}>Chờ giao hàng</Text>
            </TouchableOpacity>
          )}
          {canSetCompleted && (
            <TouchableOpacity
            ...
            >
              ...
              <Text style={styles.actionBtnText}>Xác nhận đơn hàng thành công</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
      ...
      <View style={styles.earningsCard}>
        <View style={styles.earningsRow}>
          <View style={styles.earningsItem}>
            <Text style={styles.earningsLabel}>Thu nhập ({earnings?.commissionRate || 5}%/đơn)</Text>
            <Text style={styles.earningsValue}>
              {formatCurrency(earnings?.earnings || 0)}
            </Text>
          </View>
          <View style={styles.earningsItem}>
            <Text style={styles.earningsLabel}>Đơn đã giao</Text>
            <Text style={styles.earningsValue}>{earnings?.orderCount || 0}</Text>
          </View>
        </View>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>Không có đơn hàng cần giao</Text>
          </View>
        }
      />
    </View>
